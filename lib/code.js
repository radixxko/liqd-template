...
module.exports = class Code
{
	...
	for( _for )
	{
		...
		this.code( 'let '+key+','+iterator+'='+_for.iterator+','+iterated+'='+iterator+';' );
	...
	template( template )
	{
		let scope = '{' + template.attributes.reduce(( s, a ) => s += '"'+a.attribute+'":'+(a.hasOwnProperty('expression')?a.expression:JSON.stringify(a.value))+',', '') + '}';

		this.print( 'await __TEMPLATE('+(typeof template.template==='string'?'"'+template.template.toLowerCase().replace(/\./g,'/')+'"':template.template.variable)+','+scope+')', false );
	}

	style( style )
	{
		this.print( '<style>'+Style.compile(style.source)+'</style>' );
	}
