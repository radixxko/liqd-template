...
module.exports = class Code
{
	...
	template( template )
	{
		...
		if( template.block && template.block.length )
		{
			...
			this.code( 'Object.defineProperty(render_scope,"'+this.hash+'_CONTENT",{configurable:true,async get(){let __R=[];'+this.compile_partial( template.block ).join('')+'return __R.join("")}});');
		}
		...
		for( let item of block )
		{
			...
			else if( item.statement )
			{
				...
				else if( item.statement.expression )
				{
					...
					{
						this.print( 'await '+this.hash+'_CONTENT', false );
					}
					else
					{
