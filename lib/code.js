...
module.exports = class Code
{
	...
	script( script )
	{
		...
		else
		{
			...
			{
				let script = '(function(){let attributes = {prd:"makovy"};';
	...
	compile( parsed_template, ...args )
	{
		...
		return new Function( ...args, 'return new Promise(async(resolve)=>{'+args.reverse().map(a=>'with('+a+')').join('')+'{let __D,__R=[],__JS={},__CSS={};' + this.source.join('') + 'resolve({doctype:__D,render:__R.join(""),scripts:__JS,styles:__CSS })}})' );
	}
}
