...
module.exports = class Code
{
	...
	attribute( attribute )
	{
		if( attribute.hasOwnProperty( 'expression' ))
		{
			...
			this.code( 'if('+value+'!==null && typeof '+value+' !== "object"){' );
			this.print( ' '+attribute.attribute);
			this.code( 'if(typeof '+value+'!=="object"){' );
			this.print( '=' );
			this.print( value+'?"\\""+htmlentities('+value+')+"\\"":"\\"\\""', false );
			this.code( '}}' );
		}
	...
	style( style )
	{
		...
		for( let attribute of style.attributes )
		{
			if( attribute.attribute === 'require' )
			{
				...
				requires = attribute.value.split(/\s*,\s*/).map( p =>
				{
					let path = null;

					if( p.startsWith('/') )
					{
						for( let directory of this.options.directories )
						{
							if( require('fs').existsSync( directory.replace(/\/$/,'') + p ))
							{
								path = directory.replace(/\/$/,'') + p; break;
							}
						}
	...
	script( script )
	{
		...
		{
			if( attribute.attribute === 'instance' ){ instance = true; }
		}

		for( let attribute of script.attributes )
		{
			if( attribute.attribute !== 'instance' ){ attributes.push( attribute ); }
			if( ![ 'type', 'src', 'async', 'defer' ].includes( attribute.attribute )){ scope.push(attribute); }
		}
		...
		else
		{
			...
			{
				let script = '(function(){let attributes = {};';
				...
				for( let attribute of attributes )
				{
					if( attribute.hasOwnProperty( 'expression' ))
					{
						...
						script += 'let '+value+'=('+attribute.expression+');';
						script += 'if('+value+'!==null && typeof '+value+' !== "object"){';
						script += 'attributes["'+attribute.attribute+'"]='+value+';'
						script += '}';
					}
					else{ script += 'attributes["'+attribute.attribute+'"]='+( attribute.value !== undefined ? '"'+attribute.value+'"' : 'undefined' )+';'; }
				}

				script += 'return ({attributes,source:'+source+'});})()'

				this.require( 'JS', script);
			}
			else{ this.require( 'JS', '{attributes:{},source:'+source+'}'); }
		}
	}
	...
	compile_block( block )
	{
		for( let item of block )
		{
			...
			else if( item.statement )
			{
				...
				else if( item.statement.i18n ){ this.print( 'htmlentities($('+item.statement.i18n.arguments+'))', false ); }
				...
				else if( item.statement.escaped_expression )
				{
					this.print( item.statement.escaped_expression, false );
				}
				else if( item.statement.expression )
				{
					...
					{
						this.print( 'htmlentities('+item.statement.expression+')', false );
					}
				}
			}
			else if( item.text ){ this.print( 'htmlentities('+JSON.stringify(item.text)+')', false ); }
	...
	{
		if( parsed_template.doctype )
		{
			this.print( '<!DOCTYPE ' + parsed_template.doctype + '>' );
		}

		this.compile_block( parsed_template.block );

		return new Function( ...args, 'return new Promise(async(resolve, reject)=>{'+args.reverse().map(a=>'with('+a+')').join('')+'{try{let __D,__R=[],__JS={},__CSS={};' + this.source.join('') + 'resolve({doctype:__D,render:__R.join(""),scripts:__JS,styles:__CSS })}catch(e){reject(e)}}})' );
	}
}
