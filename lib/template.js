...
module.exports = class Template
{
	constructor( options = {} )
	{
		...
		this.scope = Object.assign( Object.assign({}, this.scope || {}, { __TEMPLATE: this.render.bind( this ) }));
		this.templates = null;
	...
	_load( template )
	{
		return new Promise(( resolve, reject ) =>
		{
			require('fs').readFile( template.path, 'utf8', ( err, data ) =>
			{
				if( !err && data.length )
				{
					template.render = new Code().compile( this.parser.parse( data ), 'parameters', 'scope' );
	...
	render( template, parameters, scope = {} )
	{
		return new Promise( async( resolve, reject ) =>
		{
			try
			{
				...
				if( template )
				{
					render += await template.render( parameters, this.scope );
				}
