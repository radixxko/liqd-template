...
module.exports = class Template
{
	...
	{
		let renderer = await this._get( template.toLowerCase() );

		if( !renderer && ( template.startsWith( '<' ) || template.startsWith( '{' )))
		{
			renderer = { render: new Code( __dirname, { directories: this.options.directories }).compile( this.parser.parse( template ), 'parameters', 'scope', 'render_scope' )};
		}

		return renderer.render( parameters, scope, render_scope ).catch( err =>
		{
